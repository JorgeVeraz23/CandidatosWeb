...
import { getAllInspectionFormSlice } from 'app/redux/slices/InspectionForm/GetAllInspectionFormSlice';
...
export default function FiltrarInspeccionesRealizadas(props: Props) {
  const { 
  ...
  const filteredState = useAppSelector(state => state.getAllInspectionForm); // Nuevo estado filtrado
  ...
  useEffect(() => {
      console.log("Estado filtrado:", filteredState); // Nuevo console.log para mostrar el estado filtrado
  }, [filteredState]); // Monitorear cambios en el estado filtrado
  ...
  const handleSelectChange = (option: {value: string | number, label: string}) => {
      ...
      if(canModifyGroupQuestion){
          ...
          dispatch(getAllInspectionFormSlice.actions.filterStateById({idInspectionForm:result, canModifyGroupQuestion}))
      }
